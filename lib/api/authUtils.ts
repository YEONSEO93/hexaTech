...
export async function authorizeRequest(
  ...
  if (sessionError) {
    console.error(
      "Authorization Error: Session retrieval failed.",
      sessionError
    );
  ...
  let userRole: Database["public"]["Tables"]["users"]["Row"]["role"] | null =
    null;
  let isAdmin = false;
  if (
    allowedRoles.length > 0 ||
    (allowSelf && targetUserId !== requestingUserId)
  ) {
    ...
    if (roleError) {
      if (
        ...
        console.warn(
          `Authorization Warning: User ${requestingUserId} attempting self-operation not found in 'users' table. Proceeding without role check.`
        );
      } else {
        console.error(
          `Authorization Error: Failed to fetch role for user ${requestingUserId}.`,
          roleError
        );
        return NextResponse.json(
          { error: "Internal Server Error - Failed to verify user role" },
          { status: 500 }
        );
      }
      ...
      console.error(
        `Authorization Error: User data unexpectedly null for user ${requestingUserId} despite no roleError.`
      );
      return NextResponse.json(
        { error: "Internal Server Error - Failed to verify user role" },
        { status: 500 }
    ...
    console.log(
      `Authorization Info: Performing self-operation for user ${requestingUserId}. Role check skipped.`
    );
  }
  ...
  if (isAdmin && allowedRoles.includes("admin")) {
    authorized = true;
    console.log(
      `Authorization Info: User ${requestingUserId} authorized via admin role.`
    );
  } else if (userRole && allowedRoles.includes(userRole)) {
    authorized = true;
    console.log(
      `Authorization Info: User ${requestingUserId} authorized via role '${userRole}'.`
    );
  }
  ...
  if (!authorized && allowSelf && requestingUserId === targetUserId) {
    authorized = true;
    console.log(
      `Authorization Info: User ${requestingUserId} authorized via self-operation.`
    );
  }

  if (!authorized) {
    console.warn(
      `Authorization Failed: User ${requestingUserId} (role: ${
        userRole ?? "unknown/not fetched"
      }) attempt rejected. Allowed roles: [${allowedRoles.join(
        ", "
      )}], Allow self: ${allowSelf}, Target User ID: ${targetUserId ?? "N/A"}`
    );
    return NextResponse.json(
