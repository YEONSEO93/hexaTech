...
export async function GET(
  ...
  try {
    ...
    if (fetchError) {
      if (fetchError.code === "PGRST116") {
        return NextResponse.json(
          { error: `User with id ${userIdToFetch} not found` },
          { status: 404 }
        );
      }
    ...
    if (!userData.name) {
      ...
      return NextResponse.json(
        { error: "Invalid user data: name is required" },
        { status: 500 }
      );
    }
    ...
    const responseUser: UserResponse = {
      ...
      role: userData.role,
      company: userData.company?.name || null,
...
export async function PATCH(
  ...
  try {
    ...
    if (role !== undefined) {
      // const allowedRoles: Database["public"]["Tables"]["users"]["Row"]["role"][] =
      //   ["admin", "collaborator"];
    ...
    if (profilePhoto !== undefined) {
      if (typeof profilePhoto !== "string") {
        return NextResponse.json(
          { error: "Profile photo must be a string" },
          { status: 400 }
        );
      }
    ...
    if (updateError) {
      if (updateError.code === "PGRST116") {
        return NextResponse.json(
          { error: `User with id ${userIdToUpdate} not found for update` },
          { status: 404 }
        );
      }
