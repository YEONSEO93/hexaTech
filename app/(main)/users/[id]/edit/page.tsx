...
import { resendInviteLink } from "@/lib/supabase/auth";
...
export default function EditUser() {
  ...
  const handleResendInviteLink = async (email: string | null) => {
    if (!email) {
      setError("Email is required to resend invite link.");
      return;
    }

    const result = await resendInviteLink(email);

    if (result.success) {
      // Show success message to user
      setSuccessMessage("Invite link resent!");
    } else {
      // Show error message to user
      setError(result.error || "Failed to resend invite link.");
    }
  };
      ...
      <div className="p-8">
        ...
        {user && (<>
          <div className="space-y-6">
            ...
            <Button onClick={() => handleResendInviteLink(user?.email)}> Resend invite link</Button>
            <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
              <h2 className="font-medium text-2xl">User Details</h2>
